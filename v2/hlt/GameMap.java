...
public class GameMap {
    ...
    public ArrayList<Direction> getUnsafeMoves(final Position source, final Position destination) {
		...
		final ArrayList<Direction> possibleMovesAdd = new ArrayList<>();
        ...
        if (normalizedSource.x < normalizedDestination.x) {
			...
			possibleMovesAdd.add(Direction.WEST);
			possibleMovesAdd.add(Direction.EAST);
		}
        ...
        if (normalizedSource.y < normalizedDestination.y) {
			...
			possibleMovesAdd.add(Direction.NORTH);
			possibleMovesAdd.add(Direction.SOUTH);
		}
		
		for (Direction d : possibleMovesAdd) {
			possibleMoves.add(d);
		}
	...
	public Direction naiveNavigate(final Ship ship, final Position destination) {
		return naiveNavigate(ship, destination, true);
	}
    public Direction naiveNavigate(final Ship ship, final Position destination, final boolean targetYard) {
        ...
        for (final Direction direction : getUnsafeMoves(ship.position, destination)) {
            ...
            if (!at(targetPos).isOccupied()) {
				...
				// tabrak kalau yard dikuasai musuh
				final Ship shipInYard = at(targetPos).ship;
				if (shipInYard.owner != ship.owner) {
					at(targetPos).markUnsafe(ship);
					return direction;
				}
			}
